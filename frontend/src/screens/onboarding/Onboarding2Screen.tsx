...
// Dernier écran avant d'atterrir sur le fil d'offres ("feed").
// ════════════════════════════════════════════════════════════
...
export function Onboarding2Screen({ onNavigate }: { onNavigate: (s: Screen) => void }) {
  // Valeurs pré-remplies avec des exemples plausibles (comme si elles
  // provenaient de l'étape précédente) — purement pour la démo visuelle.
  ...
  // Index de l'heure de début dans HOURS — sert à désactiver, dans le
  // <select> "Fin", toutes les heures antérieures ou égales au début
  // (on ne peut pas finir avant/au moment où l'on commence).
  const startIdx = HOURS.indexOf(startHour);

  // Style partagé des deux <select> horaires : flèche custom (comme SelectDropdown,
  // mais ici en objet React.CSSProperties direct plutôt que via le composant partagé,
  // car ces <select> ont une taille différente de SelectDropdown.tsx).
  const dropStyle = {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollable px-6 pb-6 flex flex-col gap-7">
        ...
        {/* Zone de travail : ville + rayon en km (slider) */}
        <div>
        ...
        {/* Disponibilités : période (dates), jours de la semaine, plage horaire */}
        <div>
          ...
          <div className="bg-secondary/60 rounded-2xl border border-border p-4 mb-5">
            ...
            {/* Résumé lisible de la période, recalculé à chaque changement de date */}
            {availFrom && availTo && (
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                ...
                <select value={startHour} onChange={(e) => setStartHour(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-border bg-card text-foreground text-sm font-medium focus:border-primary transition-colors appearance-none cursor-pointer" style={dropStyle}>
                  {/* On retire la dernière heure de la liste "Début" : il faut
                      au moins un créneau après pour choisir une heure de fin. */}
              ...
              <div className="flex-1">
                ...
                <select value={endHour} onChange={(e) => setEndHour(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-border bg-card text-foreground text-sm font-medium focus:border-primary transition-colors appearance-none cursor-pointer" style={dropStyle}>
                  {/* On retire la première heure : elle ne peut pas être une
                      heure de fin. Chaque option ≤ à l'heure de début est
                      désactivée (disabled) plutôt que masquée, pour que
                      l'utilisateur comprenne pourquoi elle n'est pas cliquable. */}
      ...
      {/* Dernier bouton du parcours d'inscription : direction le fil d'offres */}
      <div className="px-6 pb-8 pt-4 border-t border-border bg-background">
        <PrimaryButton onClick={() => onNavigate("feed")}>Accéder aux offres →</PrimaryButton>
      </div>
    </div>
  );
}
